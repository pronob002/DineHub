...
class _StockFormState extends State<StockForm> {
  ...
  Map<String, dynamic> toJson() {
    ...
    Map<String, dynamic> data = {
      "Date": formattedDate,  // Convert to a string before storing
      "Item1": item1NameController.text,
      "Amount of Item1": double.tryParse(item1PriceController.text) ?? 0.0,
      "Item2": item2NameController.text,
      "Amount of Item2": double.tryParse(item2PriceController.text) ?? 0.0,
      "Item3": item3NameController.text,
      "Amount of Item3": double.tryParse(item3PriceController.text) ?? 0.0,
      "Item4": item4NameController.text,
      "Amount of Item4": double.tryParse(item4PriceController.text) ?? 0.0,
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 10),
                _buildItemPriceFormField('Other Items', otherItemsPriceController),
                SizedBox(height: 20),
                ElevatedButton(
  ...
  Widget _buildItemFormField(String label, TextEditingController controller) {
  ...
  Widget _buildItemPriceFormField(String label, TextEditingController controller) {
    return TextFormField(
      controller: controller,
      keyboardType: TextInputType.number,
      decoration: InputDecoration(
        labelText: 'Amount of $label',
        labelStyle: TextStyle(color: Colors.black),
      ...
      validator: (value) {
        if (value == null || value.isEmpty) {
          return 'Please enter the amount of $label';
        }
        return null;
      },
    );
